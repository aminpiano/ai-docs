...
2. **§1 Execution Model**: 3-Phase (scout → workload plan → write team → review team), Phase 0 skeleton structure (Value Facts, Classification Map, Cross-Reference Map, TODO Registry with ID rules, Section Numbering, File-to-Document Map, Workload Matrix, Agent Assignment Plan), Phase 1 workload-based parallel writing rules (writers must use skeleton classifications verbatim — never re-derive; heavy/critical docs may be split into fragments with one doc owner/integrator), Phase 2 workload-based review + cross-check (reviewers are assigned by risk/coverage, not fixed document bundles; cross-checker must self-verify their own 00_INDEX output)
